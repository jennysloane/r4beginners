...
In this lesson we will review some of the basic syntax of R, including several common operators, including the following:
- assignment operator (<-)
...
- > greater than operator
- < less than operator
- Is x greater than y? Remember, we have x assigned to 10 and y assigned to 25. So is 10 greater than 25? True or False? 
```{r}
x > y
```
...
- So, let's test the same line of code we had in the last example but with an | operator instead of an & operator and see what gets returned
```{r}
z > 50 | z > 200
```
